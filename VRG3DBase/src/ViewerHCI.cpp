...
ViewerHCI::ViewerHCI(EventMgrRef eventMgr, GfxMgrRef gfxMgr)
{
  ...
  _rockingDir = 1.0;
  _totalAngle = 0.0;
  _rotAngularVel = MinVR::ConfigVal("ViewerHCI_RockingSpeed", 0.35, false);
  _rockingAngle = MinVR::ConfigVal("ViewerHCI_RockingAngle", 0.8, false);

  _boundingSphere = Sphere(Vector3::zero(), MinVR::ConfigVal("ViewerHCI_TrackballRad", 0.55, false));

  _panAmt = MinVR::ConfigVal("ViewerHCI_PanIncrement", 0.05, false);
...
void
ViewerHCI::trackballMove(MinVR::EventRef e)
{
...
void
ViewerHCI::trackballOff(MinVR::EventRef e)
{
}


void
ViewerHCI::toggleRocking(MinVR::EventRef e)
{
  _rocking = !_rocking;
  if (_rocking) {
    _lastRotTime = -1;
...
void
ViewerHCI::panLeft(MinVR::EventRef e)
{
...
void
ViewerHCI::panRight(MinVR::EventRef e)
{
  CoordinateFrame t(Vector3(-_panAmt,0,0));
  _gfxMgr->setRoomToVirtualSpaceFrame(_gfxMgr->getRoomToVirtualSpaceFrame() * t);
}
...
void
ViewerHCI::panUp(MinVR::EventRef e)
{
...
void
ViewerHCI::panDown(MinVR::EventRef e)
{
  CoordinateFrame t(Vector3(0,_panAmt,0));
  _gfxMgr->setRoomToVirtualSpaceFrame(_gfxMgr->getRoomToVirtualSpaceFrame() * t);
}
...
void
ViewerHCI::dollyIn(MinVR::EventRef e)
{
...
void
ViewerHCI::dollyOut(MinVR::EventRef e)
{
  CoordinateFrame t(Vector3(0,0,_panAmt));
  _gfxMgr->setRoomToVirtualSpaceFrame(_gfxMgr->getRoomToVirtualSpaceFrame() * t);
}
...
void
ViewerHCI::scaleUp(MinVR::EventRef e)
{
  _gfxMgr->setRoomToVirtualSpaceScale(0.95 * _gfxMgr->getRoomToVirtualSpaceScale());
}

void
ViewerHCI::scaleDown(MinVR::EventRef e)
{
  _gfxMgr->setRoomToVirtualSpaceScale(1.05 * _gfxMgr->getRoomToVirtualSpaceScale());
}

void
ViewerHCI::speedUpRocking(MinVR::EventRef e)
{
  _rotAngularVel += 0.05;
}

void
ViewerHCI::slowDownRocking(MinVR::EventRef e)
{
  _rotAngularVel -= 0.05;
}

void
ViewerHCI::incRockingAngle(MinVR::EventRef e)
{
  _rockingAngle += 0.05;
}

void
ViewerHCI::decRockingAngle(MinVR::EventRef e)
{
  _rockingAngle -= 0.05;
}
