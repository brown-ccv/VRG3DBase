...
GfxMgr::GfxMgr(G3D::RenderDevice *renderDevice, MinVR::ProjectionVRCameraRef camera)
{
...
GfxMgr::drawFrame(Vector3 lookVec)
{
  ...
  if (bgTex.notNull()) {
    ...
    double scale = MinVR::ConfigVal("BackgroundImageRepeat", 1.0);
...
GfxMgr::getDefaultFont()
{
  if (_defaultFont.isNull()) {
    std::string ffile = MinVR::ConfigVal("GfxMgr_Font", "$(G)/src/SFVRG3D/vrg3dsvn-latest/vrg3d/share/eurostyle.fnt", false);
